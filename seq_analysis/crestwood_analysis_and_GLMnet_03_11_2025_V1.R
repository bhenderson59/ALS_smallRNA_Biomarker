...
#cd /cluster/home/broberts/Ben_miRNA_work/ALSbiomarker/CrestwoodSeqAnalysisClean
...
wrkspc_img <- "crestwood_Analysis_03_11_2025.rd"
#load(wrkspc_img)
...
write.csv(raw_cnts_mat, "ALS_Crest_seqcenter_Groups1_3_biomarker_raw_cnts_mat.csv")
...
pdf(file="Crestwood_totMiRNA_cnts.pdf",width=10,height=2.5)
par(bty="l")
...
pdf(file="Crestwood_miRNACntsVFracALS.pdf",width=4,height=2.5)
par(bty="l")
par(mai=c(0.5,0.5,0.03,0.1))
par(mgp=c(2.6,0.6,0))
plot(x=sortTotCnts,
     y=fracALS,
     xlab="aligned small RNA reads",
...
mask_486_3p <- grep("hsa_miR_486_3p",as.character(rownames(raw_cnts_mat)))
...
tot_486_3p_cpm <- colSums(cpm_mat[mask_486_3p,])

orderBy486Cpm <- order(tot_486_3p_cpm)

pdf(file="Crestwood_miR_486_3p_CPM_cnts.pdf",width=10,height=2.5)
par(bty="l")
par(mai=c(0.8,0.7,0.1,0.1))
par(mgp=c(2.6,0.6,0))
...
sample_key_df$log486cpm <- log(colSums(cpm_mat[mask_486_3p,]))
...
write.csv(data.frame(covar=rownames(pc_explore_mat_logCpm),pc_explore_mat_logCpm),
          file="Crestwood_plasma_logCPM_PC_explore_res.csv",row.names=F,quote=F)



#no cutoffs on total cnts and miR-486-3p because their in range

#tot cnts
totCntsQs <- quantile(cnts_colSums)

#there an inflection point in plot fo tot cnts vs frac ALS at 1e6
maskPassCnts <- which(cnts_colSums>=3e6)
...
maskPass486 <- which(tot_486_3p_cpm<=cpm486Qs[5])
length(maskPass486)
...
batchCatCnts <- lapply(uniqPassCntsand486Batches,function(batchX){
  ...
  nCtl <- length(which(diseaseX=="CTL"))
...
medianMirCnts[grep("miR_206",rownames(pass_cnts_mat))]


pdf(file="Crestwood_plasma_medianMiRCnts_Hist.pdf",
    width=5,
    height=2.5)
...
modMatrixForComBat <- model.matrix(~.,data=pass_sample_key[,c(2,3,4)])
...
write.csv(data.frame(covar=rownames(pc_explore_mat_PASS_logCpm),pc_explore_mat_PASS_logCpm),
          file="Crestwoodl_plasma_PASS_logCPM_PC_explore_res.csv",row.names=F,quote=F)
...
write.csv(allSummResDf,file="Crestwood_regress_Summary_selected_Covars.csv",
          row.names=FALSE,
          quote=FALSE)
...
residSDs[grep("miR_206",colnames(lmResids))]
...
png(file="Crestwood_scaled_Covar_Resids_All_heatmap.png",
    width=10,
    height=8,
    units="in",
    res=1000,
...
write.csv(diseaseVsResidsLmResDf,
          file="Crestwood_LM_regress_Summary_Resids_ALS_vs_cntrl.csv",
          row.names=FALSE,
          quote=FALSE)

diseaseVsResidsLmResDf[grep("miR_206",diseaseVsResidsLmResDf$miRNA),]
...
png(filename = "Crestwood_ALS_LmResids_lasso_plot.png",
...
write.csv(lamdaPickedlasso_coeff_df,file="Crestwood_ALS_AllSamples_resids_lambda_picked_features.csv",row.names=F,quote=F)
...
plotVals <- scaledResids[,grep("hsa_miR_206",colnames(scaledResids))]
...
fileX <- paste("Crestwood_MiR206_resids_boxplot.pdf",sep="_")
